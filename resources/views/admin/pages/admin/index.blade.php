	      				...
	      				<div class="card-content collapse show">
	      					<div class="card-body card-dashboard">
	      						...
	      						<table class="table table-striped table-bordered dataex-html5-export">
	      							...
	      							<tbody>
										@if(count($admins) > 0)
											@foreach($admins as $k=>$u)
												<tr>
													<td>{{ ++$k }}</td>
													<td>{{ $u->name }}</td>
													<td>{{ $u->email }}</td>
													<td>
													{{ optional(App\Models\Role::find($u->role_id))->name ?? 'Deleted' }}
													</td>
													<td>
														@if($u->status == '1')
													...
													<td>
													    <a href="{{ route('admin.admins.edit', $u->id) }}" data-toggle="tooltip" data-placement="top" title="Edit">
															<button type="button" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
														</a>&nbsp;	
														@if($u->role_id != '1')							
															<button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('admin.admins.delete', $u->id) }}')" data-toggle="tooltip" data-placement="top" title="Delete">
