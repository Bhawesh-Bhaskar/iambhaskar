        ...
        <div class="content-header row">
			...
			<div class="col-md-4 text-right">
				<a href="{{ route('admin.blog.create') }}">
					<button class="btn btn-primary"><i class="fa fa-plus"></i> Add Blog</button>
                </a>
			</div>  
        </div>
	      							...
	      							<tbody>
	      								@if(count($blogs) > 0)
											...
											<tr>
												...
												<td>
													@if($u->image)
									                    <img src="{{ asset('assets/img/blogs/'.$u->image) }}" style="width: 100px; height: 70px;">
									                  @else
												...
												<td>
													@foreach($admins as $admin)
														@if($admin->id == $u->added_by)
														    {{ $admin->name }}
														@endif
													@endforeach
												</td>
												<td>
													<a href="{{ route('admin.blog.edit', $u->slug) }}" data-toggle="tooltip" data-placement="top" title="Edit">
														<button type="button" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
													</a>&nbsp;							
													<button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('admin.blog.delete', $u->id) }}')" data-toggle="tooltip" data-placement="top" title="Delete">
